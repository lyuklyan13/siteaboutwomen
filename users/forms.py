...
class RegisterUserForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email=email).exists():
            raise forms.ValidationError("Такий E-mail вже зареєстрований!")
        return email
...
class UserPasswordChangeForm(PasswordChangeForm):
    ...
    new_password2 = forms.CharField(label="Підтвердження пароля", widget=forms.PasswordInput(attrs={'class': 'form-input'}))
